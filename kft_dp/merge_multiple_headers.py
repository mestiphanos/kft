...
-    formats multiple headers
    ...
    def __init__(self,df,col_start_idx = 0,col_start_val = 'ተ.ቁ'):
        self.df = df
        print(col_start_idx,col_start_val)
    ...
    def get_df(self,filepath,s_name,header_start=0,header_end=0):
        """
        Get the dataframe
        """
        if header_start:
        ...
        # self.df.columns = self.df.loc[self.header_start:self.header_end-1,:].values.tolist()
        # self.df.drop(range(0,self.header_end), axis=0, inplace=True)
        header = list(range(self.header_start,self.header_end))
        self.df = return_sheet_data(filepath,s_name,header)
        # self.df = pd.read_excel(filename,engine='openpyxl',header=,sheet_name=s_name)
    ...
    def set_header_idx(self,col_start_idx,col_start_val):
        if not self.df.empty:
            ...
            if not self.df[self.df[col_start_idx] == col_start_val].empty:
                ...
                self.header_start = self.df.index[self.df[col_start_idx] == col_start_val].tolist()[0]
